...
import org.rulez.magwas.zenta.templates.impl.model.ZentamateModelTemplate;
 ...
 */
public class SaveZentamateModelAsTemplateWizard extends Wizard {
    
    private IZentamateModel fModel;
    
    private SaveZentamateModelAsTemplateWizardPage fPage1;
    ...
    public SaveZentamateModelAsTemplateWizard(IZentamateModel model) {
        setWindowTitle(Messages.SaveZentamateModelAsTemplateWizard_0);
        fModel = model;
    ...
    public void addPages() {
        fPage1 = new SaveZentamateModelAsTemplateWizardPage(fModel, fTemplateManager);
        addPage(fPage1);
        fPage2 = new SaveZentamateModelAsTemplateToCollectionWizardPage(fTemplateManager);
        addPage(fPage2);
    }
    ...
    public boolean performFinish() {
        ...
        if(fZipFile.exists()) {
            boolean result = MessageDialog.openQuestion(Display.getCurrent().getActiveShell(),
                    Messages.SaveZentamateModelAsTemplateWizard_1,
                    NLS.bind(Messages.SaveZentamateModelAsTemplateWizard_2, fZipFile.getPath()));
            if(!result) {
                return false;
            }
        }
            ...
            public void run() {
                try {
                    ...
                    Display.getCurrent().asyncExec(new Runnable() { // Display after wizard closes
                        public void run() {
                            MessageDialog.openError(getShell(), Messages.SaveZentamateModelAsTemplateWizard_3, ex.getMessage());
                        }
                    });
                }
            }
    ...
    private String createManifest() throws IOException {
        ...
        // Type
        root.setAttribute(ITemplateXMLTags.XML_TEMPLATE_ATTRIBUTE_TYPE, ZentamateModelTemplate.XML_TEMPLATE_ATTRIBUTE_TYPE_MODEL);
    ...
    private File saveModelToTempFile() throws IOException {
        ...
        // Copy the model
        IZentamateModel tempModel = EcoreUtil.copy(fModel);
