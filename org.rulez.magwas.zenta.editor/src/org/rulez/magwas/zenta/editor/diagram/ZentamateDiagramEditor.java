...
/**
 * Zentamate Diagram Editor
 * 
 * @author Phillip Beauvoir
 */
public class ZentamateDiagramEditor extends AbstractDiagramEditor
implements IZentamateDiagramEditor {
    
    /**
     * Palette
     */
    private ZentamateDiagramEditorPalette fPalette;
    ...
    @Override
    public ZentamateDiagramEditorPalette getPaletteRoot() {
        if(fPalette == null) {
            fPalette = new ZentamateDiagramEditorPalette();
            setPaletteViewpoint();
        }
        return fPalette;
    }
    ...
    protected void configureGraphicalViewer() {
        ...
        viewer.setEditPartFactory(new ZentamateDiagramEditPartFactory());
        
        // Set Model
        viewer.setContents(getModel());
        
        // Native DnD
        viewer.addDropTargetListener(new ZentamateDiagramTransferDropTargetListener(viewer));
    }
    ...
    protected void registerContextMenu(GraphicalViewer viewer) {
        MenuManager provider = new ZentamateDiagramEditorContextMenuProvider(viewer, getActionRegistry());
        viewer.setContextMenu(provider);
        getSite().registerContextMenu(ZentamateDiagramEditorContextMenuProvider.ID, provider, viewer);
    }
    
    @Override
    public void selectElements(IZentamateElement[] elements) {
        ...
        for(IZentamateElement element : elements) {
            ...
            if(ConnectionPreferences.useNestedConnections() && element instanceof IRelationship) {
                for(IDiagramModelZentamateObject[] list : DiagramModelUtils.findNestedComponentsForRelationship(getModel(), (IRelationship)element)) {
    ...
    protected void eCoreModelChanged(Notification msg) {
        ...
        if(msg.getEventType() == Notification.SET) {
            ...
            if(msg.getNotifier() == getModel() && msg.getFeature() == IZentamatePackage.Literals.ARCHIMATE_DIAGRAM_MODEL__VIEWPOINT) {
                setViewpoint();
            }
        }
    }
    ...
    public String getSearchExpression(Object target) {
        return Messages.ZentamateDiagramEditor_0;
    }
}
