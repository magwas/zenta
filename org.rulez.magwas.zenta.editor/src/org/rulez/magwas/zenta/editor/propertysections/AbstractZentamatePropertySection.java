 ...
 */
public abstract class AbstractZentamatePropertySection extends AbstractPropertySection {
    ...
    protected boolean isAlive() {
        return (getEObject() != null) && 
        	   (getEObject() instanceof IZentamateModel) || (getEObject().eContainer() != null);
    }
    ...
    protected PropertySectionTextControl createNameControl(Composite parent, String hint) {
        // Label
        createLabel(parent, Messages.AbstractZentamatePropertySection_0, ITabbedLayoutConstants.STANDARD_LABEL_WIDTH, SWT.CENTER);

        // Text
        Text textControl = createSingleTextControl(parent, SWT.NONE);
        
        PropertySectionTextControl textName = new PropertySectionTextControl(textControl, IZentamatePackage.Literals.NAMEABLE__NAME) {
            @Override
            protected void textChanged(String oldText, String newText) {
                if(isAlive()) {
                    fIsExecutingCommand = true;
                    getCommandStack().execute(new EObjectFeatureCommand(Messages.AbstractZentamatePropertySection_1 + " " + oldText, getEObject(), //$NON-NLS-1$
    ...
    protected PropertySectionTextControl createDocumentationControl(Composite parent, String hint) {
        // Label
        createLabel(parent, Messages.AbstractZentamatePropertySection_2, ITabbedLayoutConstants.STANDARD_LABEL_WIDTH, SWT.NONE);
        
        // Text
        StyledTextControl styledTextControl = createStyledTextControl(parent, SWT.NONE);
        
        PropertySectionTextControl textDoc = new PropertySectionTextControl(styledTextControl.getControl(), IZentamatePackage.Literals.DOCUMENTABLE__DOCUMENTATION) {
            @Override
            protected void textChanged(String oldText, String newText) {
                if(isAlive()) {
                    fIsExecutingCommand = true;
                    getCommandStack().execute(new EObjectFeatureCommand(Messages.AbstractZentamatePropertySection_3, getEObject(),
                            IZentamatePackage.Literals.DOCUMENTABLE__DOCUMENTATION, newText));
                    fIsExecutingCommand = false;
                }
            }
        };
