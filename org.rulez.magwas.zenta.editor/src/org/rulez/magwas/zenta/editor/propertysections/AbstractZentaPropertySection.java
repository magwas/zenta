 ...
 */
public abstract class AbstractZentaPropertySection extends AbstractPropertySection {
    ...
    protected boolean isAlive() {
        return (getEObject() != null) && 
        	   (getEObject() instanceof IZentaModel) || (getEObject().eContainer() != null);
    }
    ...
    protected PropertySectionTextControl createNameControl(Composite parent, String hint) {
        // Label
        createLabel(parent, Messages.AbstractZentaPropertySection_0, ITabbedLayoutConstants.STANDARD_LABEL_WIDTH, SWT.CENTER);

        // Text
        Text textControl = createSingleTextControl(parent, SWT.NONE);
        
        PropertySectionTextControl textName = new PropertySectionTextControl(textControl, IZentaPackage.Literals.NAMEABLE__NAME) {
            @Override
            protected void textChanged(String oldText, String newText) {
                if(isAlive()) {
                    fIsExecutingCommand = true;
                    getCommandStack().execute(new EObjectFeatureCommand(Messages.AbstractZentaPropertySection_1 + " " + oldText, getEObject(), //$NON-NLS-1$
    ...
    protected PropertySectionTextControl createDocumentationControl(Composite parent, String hint) {
        // Label
        createLabel(parent, Messages.AbstractZentaPropertySection_2, ITabbedLayoutConstants.STANDARD_LABEL_WIDTH, SWT.NONE);
        
        // Text
        StyledTextControl styledTextControl = createStyledTextControl(parent, SWT.NONE);
        
        PropertySectionTextControl textDoc = new PropertySectionTextControl(styledTextControl.getControl(), IZentaPackage.Literals.DOCUMENTABLE__DOCUMENTATION) {
            @Override
            protected void textChanged(String oldText, String newText) {
                if(isAlive()) {
                    fIsExecutingCommand = true;
                    getCommandStack().execute(new EObjectFeatureCommand(Messages.AbstractZentaPropertySection_3, getEObject(),
